...
def search_button(win):
    #logger = get_logger('invoice_scrapper')
    #drive = open_browser(win.logger, 'firefox')
    #open_site(drive, sefaz_ba_site, win.logger)
    drive = win.drive
    ...
    if wait_html_tab(drive, 10, 'btn_aba_emitente', "Site Lento, Aguardar pela Visualização dos Emmitente da Nota?"):
        return #se optou por não aguardar sai
    suplier_data = get_supplier_data(drive)
    win.logger.info(f'Emitente: {suplier_data["razao_social"]} Cnpj: {suplier_data["cnpj"]}')
    ...
    session = Session()
    try:
        put_suplier_data_in_db(suplier_data, session)
        put_invoice_data_in_db(invoice_data, session)
        put_bill_data_in_db(bill_data, session)
        put_total_data_in_db(total_data, session)
        put_delivery_data_in_db(delivery_data, session)
        put_products_data_in_db(products_data, session)
...
def put_delivery_data_in_db(delivery_data, session):
...
def put_total_data_in_db(total_data, session):
...
def put_suplier_data_in_db(suplier_data, session):
    if suplier_data:
        query = session.query(Emitente).filter(Emitente.cnpj == suplier_data['cnpj'])    
        if query.count() == 0:
            emitente = Emitente()
            emitente.cnpj = suplier_data['cnpj']
            emitente.razao_social = suplier_data['razao_social']
            emitente.nm_fantasia = suplier_data['nm_fantasia']
            emitente.endereco = suplier_data['endereco']
            emitente.bairro = suplier_data['bairro']
            emitente.cep = suplier_data['cep']
            emitente.cd_municipio = suplier_data['cd_municipio']
            emitente.cd_municipio_ocorrencia = suplier_data['cd_municipio_ocorrencia']
            emitente.telefone = suplier_data['telefone']
            emitente.sg_uf = suplier_data['sg_uf']
            emitente.cd_pais = suplier_data['cd_pais']
            emitente.insc_estadual = suplier_data['insc_estadual']
            emitente.cnae_fiscal = suplier_data['cnae_fiscal']
            emitente.ds_regime_tributario = suplier_data['ds_regime_tributario']
            emitente.insc_estadual_substituto = suplier_data['insc_estadual_substituto']
            emitente.insc_municipal = suplier_data['insc_municipal'] 
            session.add(emitente)

def match_id_produto(product_data, session):
    ...
    if product_data: #tem dados?
        if product_data['cd_ean_prod_serv'] == 'SEM GTIN': #produto sem código de barras?
            ...
            if query_no_ean.count() == 1: #se estiver, nada a fazer
               return
...
def put_product_gtin_data(product_data, session):
    ''' 
...
def put_products_data_in_db(products_data, session): 
    if products_data:
        ...
        for product_data in products_data:
            ...
            if query.count() == 0:
                put_product_gtin_data(product_data, session)
                ...
                session.add(produto)
                match_id_produto(product_data, session)
            query = session.query(ProdutoServico)


def put_bill_data_in_db(bill_data, session):
    if bill_data['ds_forma_pagamento']:
        ...
        if query.count() == 0:
            ...
            #campo a ser inserido na tabela ->numero_autorizacao'


def put_invoice_data_in_db(invoice_data, session):
    if invoice_data:
